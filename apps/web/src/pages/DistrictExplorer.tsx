...
export default function DistrictExplorer() {
  ...
  return (
    <div className="mx-auto flex min-h-full max-w-[1680px] flex-col gap-6 p-4 sm:p-6 lg:p-8">
      <PageHeader
        eyebrow="District Prioritization"
        title="District Action Explorer"
        description="Inspect root cause, intervention priority, escalation posture, and recommended institutional response for any monitored district."
      />
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)]">
              ...
              <tbody>
                {filtered.map((district) => (
                  <tr key={district.id} className="border-t border-border text-text-main">
                    ...
                    <td className="px-3 py-4 text-xs font-semibold uppercase tracking-[0.18em] text-text-muted">
                      {getPriorityLabel(district.riskLevel)}
                    </td>
        ...
        <div className="min-w-0 space-y-6 xl:sticky xl:top-6 xl:max-h-[calc(100dvh-8rem)] xl:overflow-y-auto xl:pr-1">
          <div className="min-w-0 overflow-hidden rounded-3xl border border-border bg-surface-1 p-5 shadow-[0_16px_40px_rgba(20,44,31,0.08)]">
            ...
            {!loadingDetail && !selectedDistrict && <p className="mt-4 text-sm text-text-muted">Pick a district to inspect the full RiskReport, intervention recommendation, and audit trail fragment.</p>}
            {selectedDistrict && (
              <div className="mt-4 space-y-4">
                <div className="rounded-2xl bg-surface-2 p-4">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    ...
                    <SummaryMetric label="Intervention Priority" value={getPriorityLabel(selectedDistrict.riskReport.riskCategory)} />
                    <SummaryMetric label="Escalation" value={selectedDistrict.escalate ? 'Required' : 'Not required'} />
                    <SummaryMetric label="Recommended Action" value={selectedDistrict.riskReport.recommendedAction} />
                    <SummaryMetric label="Data Freshness" value={`${selectedDistrict.dataFreshnessDays} day(s)`} />
                  </div>
                </div>
                <div className="rounded-2xl border border-border bg-background/60 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-text-muted">Institutional Briefing</p>
                  <p className="mt-2 text-sm font-semibold text-text-main">Recommended institutional response</p>
                  <p className="mt-1 text-sm leading-6 text-text-muted">{selectedDistrict.advisoryInstitutional}</p>
                  <p className="mt-4 text-sm font-semibold text-text-main">Warnings and escalation notes</p>
                  <p className="mt-1 text-sm leading-6 text-text-muted">
                    {selectedDistrict.warnings.join(' • ') || selectedDistrict.escalationReason || 'No additional warnings'}
                  </p>
                </div>
              </div>
            )}
...
function getPriorityLabel(riskLevel: AnalysisResponse['riskReport']['riskCategory']) {
  if (riskLevel === 'CRITICAL') return 'Immediate';
  if (riskLevel === 'HIGH') return 'High';
  if (riskLevel === 'MEDIUM') return 'Targeted';
  return 'Routine';
}
